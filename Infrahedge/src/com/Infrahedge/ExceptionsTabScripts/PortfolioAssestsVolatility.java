...
public class PortfolioAssestsVolatility extends BaseTest {
		...
		for (int j = 2; j >= 0; j--) {
			...
			try {
				...
				for (int i = 1; i <= PortfolioRows.size(); i++) {
					Thread.sleep(2000);
					WebElement PortfolioElement = driver
				...
				try {
					...
					WebElement secondLastElement = driver
							.findElement(By.xpath("//table/tbody/tr[18]/td[4]/div/div/span"));
					...
					for (int s = 1; s <= count; s++) {
						...
						if (text17 != text18) {
							...
							Thread.sleep(2000);
							WebElement PortfolioElement = driver
									.findElement(By.xpath("//*[@id='QV019']//table/tbody/tr[18]/td[4]"));
							PortfolioElement.click();

							Thread.sleep(6000);
							WebElement expandAll = driver.findElement(
									By.xpath("//*[@class='lui-icon lui-icon--plus gray pivot-expand-colapse']"));
	...
	public void fecthPortfolioAssets() throws InterruptedException, IOException, AWTException {
		...
		/*
		 * String ip2="IP2"; readassetSubAssetValue(ip2,category,kpi); String ip3="IP3";
		 * readassetSubAssetValue(ip3,category,kpi); String ip4="IP4";
		 * readassetSubAssetValue(ip4,category,kpi); String ip5="IP5";
