...
public class StressPage extends BaseTest  {
	...
	public void verifyBIbutton() throws InterruptedException, IOException
	{
		...
		Assert.assertTrue(BIbutton.isSelected());
	...
	public void assetPage() throws InterruptedException {
		...
		Assert.assertTrue(Asset.equals("Asset Class"));
...
//			readColNames.add(Col12);
	...
	public void uncheckdefaultcolumn() throws InterruptedException
	{
		...
		try {
			...
			Assert.assertTrue(Integer.parseInt(check)==0);
	...
	public void filterOptiontest()
	{
		...
		try {
			...
			while(it.hasNext())
			{
				if(it.next().getText().equals("Dynamic"))
				{
					isFound = true;
				}
				else
	...
	//Verify Filter Risk Profile
	
	public void filterRiskProfile()
	{
	...
	//filterICon.click();
		try {
			...
			riskprofile.add("Dynamic"); riskprofile.add("IP1"); riskprofile.add("IP2");
			...
			System.out.println("Risk profile is displayed with 6 Risk profiles ");
			test.log(Status.INFO, "Risk profile is displayed with 6 Risk profiles");
	...
	public void assetPageLayout() throws InterruptedException, IOException
	{
			...
			Assert.assertTrue(ExpandAll.isDisplayed() && CollapseAll.isDisplayed());
			...
			//verify Settings Ellipsis
				Assert.assertTrue(settingsButton.isDisplayed() && ellipsisButton.isDisplayed());
	...
	public void filterPortManager()
	{
		...
		try {
			...
			Thread.sleep(3000);
			Assert.assertTrue(select.equals("Portfolio Manager - " + subselect));
	...
	public void filterstrategy()
	{
		...
		//	WebElement strategy = driver.findElement(By.xpath("(//*[text()='Strategy' and @title='Strategy'])[3]"));
			...
			Thread.sleep(1000);
			Assert.assertTrue(select.equals("Strategy - " + subselect));
	...
	public void filterSearch()
	{
		...
		try {
			...
			if(driver.findElement(By.xpath("//*[text()='There are no matches for your search.']")).isDisplayed())
			{
